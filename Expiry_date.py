...
class ExpiryDateViewerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Expiry Date Viewer")

        self.label = ttk.Label(root, text="Enter a date:")
        self.label.pack(pady=10)

        self.date_entry = ttk.Entry(root)
        self.date_entry.pack()

        self.view_button = ttk.Button(root, text="View Expiry Dates", command=self.view_expiry_dates)
    ...
    def view_expiry_dates(self):
        ...
        try:
            ...
            # Check if the entered date is a Saturday or Sunday
            if date_obj.weekday() in [5, 6]:
                self.output_text.delete("1.0", tk.END)
                self.output_text.insert(tk.END, "You cannot enter a date on Saturday or Sunday.")
                return
            
            expiry_dates = self.nse_archives.expiry_dates(date_obj)
            previous_expiry_dates = [date for date in expiry_dates if date < date_obj]
            upcoming_expiry_dates = [date for date in expiry_dates if date > date_obj]

            output_text = f"Expiry Dates for {input_date}:\n\n"
            output_text += "Previous Expiry Dates:\n"
            for date in previous_expiry_dates:
                output_text += date.strftime("%Y-%m-%d") + "\n"

            output_text += "\nUpcoming Expiry Dates:\n"
            for date in upcoming_expiry_dates:
                output_text += date.strftime("%Y-%m-%d") + "\n"
...
class NSEArchives:
    ...
    def expiry_dates(self, dt):
        txt = self.bhavcopy_fo_raw(dt)
        rows = txt.split("\n")
